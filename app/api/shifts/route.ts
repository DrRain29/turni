...
export async function GET(request: NextRequest) {
  try {
    ...
    // Query per i turni
    let shiftsQuery = supabase.from("shifts").select("*").eq("status", "scheduled").order("date", { ascending: true })
    ...
    if (shiftsError) {
      ...
      return NextResponse.json({ error: "Errore nel caricamento dei turni", details: shiftsError }, { status: 500 })
    }
    ...
    if (usersError) {
      ...
      return NextResponse.json({ error: "Errore nel caricamento degli utenti", details: usersError }, { status: 500 })
    }
    ...
    if (shiftTypesError) {
      ...
      return NextResponse.json(
        { error: "Errore nel caricamento dei tipi di turno", details: shiftTypesError },
        { status: 500 },
      )
    }
    ...
    console.log("Shift types loaded:", shiftTypes?.length || 0)
    ...
    const enrichedShifts = shifts?.map((shift) => {
      ...
      // Debug per il primo turno
      if (shift === shifts[0]) {
        console.log("Enriched shift sample:", enriched)
        console.log("User found for shift:", user)
        console.log("Shift type found for shift:", shiftType)
      }
    ...
    return NextResponse.json({ error: "Errore del server", details: error }, { status: 500 })
  }
}
